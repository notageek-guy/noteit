...
export async function DELETE(
  ...
  try {
    ...
    const count = await prisma.documents.count({
      where: { publicId: params.publicId, ownerId: userId },
    });
    if (count === 0) return new Response("Forbidden", { status: 403 });
    await prisma.documents.delete({ where: { publicId: params.publicId } });
